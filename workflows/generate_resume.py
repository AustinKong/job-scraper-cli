...
import weasyprint
import ui
from models import Profile, Experience, ResumeContext
...
def generate_resume():
  ...
  resume_context = ResumeContext(
    ...
    skills=[], # TODO: Let AI extract skills from experiences
    education=[ResumeContext.ResumeEducation(**edu.model_dump()) for edu in profile.education],
    work_experience=[
      ResumeContext.ResumeExperience(
        title=exp.title,
        company=exp.company,
      ...
      ) for exp in experiences if exp.type == "work" # TODO: Let AI customize by keywords
    ],
    projects=[
      ResumeContext.ResumeExperience(
        title=exp.title,
        ...
        location=exp.location,
        bullets=[b.text for b in exp.bullets]
      ) for exp in experiences if exp.type == "project"
    ],
    awards=[],
  )
