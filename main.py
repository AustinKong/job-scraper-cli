...
get_listings_chain = \
  with_spinner(url_dedup.map(), "URL deduping") | \
  with_spinner(RunnableBranch( # Scrape and format only if not found by URL dedup, we do this here because fuzzy dedup is extremely not reliable
...
choices = [questionary.Choice(f"{l['title']}, {l['company']} ({l['dedup_conf']})", value=l, checked=(l["dedup_conf"] < DEDUP_CONF_THRESHOLD)) for l in listings]
to_save = questionary.checkbox(f"Found {len([l for l in listings if l['dedup_conf'] < DEDUP_CONF_THRESHOLD])} unique listings out of {len(urls)} URLs. Select to save and generate:", choices=choices).ask()
